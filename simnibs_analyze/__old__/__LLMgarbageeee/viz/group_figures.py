...
class GroupVisualizer:
    ...
    def roi_definition(
        ...
        for ax, axis, slice_idx, name in zip(axes, [0, 1, 2], slices, axes_names):
            ...
            if template_data is not None:
                background_slice = extract_slice(template_data, axis=axis, slice_idx=slice_idx)
        ...
        fig.suptitle(f'ROI Definition - {roi_name} (MNI Space)', 
    ...
    def stim_vs_opti_comparison(
        ...
        if not pivot.empty and 'simulation' in pivot.columns and 'optimization' in pivot.columns:
            plot_paired_data(
                axes[1],
        ...
        fig.suptitle(f'Group Level Analysis - {roi_name}', 
                     fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def multi_roi_comparison(
        ...
        for ax, roi_name in zip(axes, roi_names):
            ...
            plot_boxplot_comparison(
                ...
                title=roi_name,
                ylabel=f'{metric.capitalize()} E-field (V/m)' if roi_name == roi_names[0] else ''
            )
