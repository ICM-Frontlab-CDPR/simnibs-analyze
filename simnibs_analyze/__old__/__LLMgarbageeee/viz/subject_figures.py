...
class SubjectVisualizer:
    ...
    def segmentation_check(
        ...
        for ax, slice_idx in zip(axes, slices):
            ...
            plot_segmentation_overlay(ax, t1_slice, seg_slice, title=f'Slice {slice_idx}')
        
        fig.suptitle(f'Segmentation Check - Subject {subject_id}', 
    ...
    def preprocessing_effect(
        ...
        # Histogrammes
        plot_histogram(axes[0], raw_values, title='Raw E-field', bins=bins, color='blue')
        plot_histogram(axes[1], prep_values, title='Preprocessed E-field', bins=bins, color='orange')
        
        fig.suptitle(f'Preprocessing Effect - {subject_id} - {roi_name}', 
                     fontsize=14, fontweight='bold')
        plt.tight_layout()
